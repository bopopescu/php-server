...
---
module: redis
short_description: Various redis commands, subordinate and flush
description:
...
options:
    command:
        description:
            ...
            - C(subordinate) sets a redis instance in subordinate or main mode.
        required: true
        choices: [ config, flush, subordinate ]
    login_password:
        description:
    ...
    main_host:
        description:
            - The host of the main instance [subordinate command]
    main_port:
        description:
            - The port of the main instance [subordinate command]
    subordinate_mode:
        description:
            - the mode of the redis instance [subordinate command]
        default: subordinate
...
notes:
   ...
   - If the redis main instance we are making subordinate of is password protected
     this needs to be in the redis.conf in the mainauth variable
...
EXAMPLES = '''
- name: Set local redis instance to be subordinate of melee.island on port 6377
  redis:
    command: subordinate
    main_host: melee.island
    main_port: 6377

- name: Deactivate subordinate mode
  redis:
    command: subordinate
    subordinate_mode: main
...
def set_subordinate_mode(client, main_host, main_port):
    try:
        return client.subordinateof(main_host, main_port)
    except Exception:
        return False


def set_main_mode(client):
    try:
        return client.subordinateof()
    except Exception:
        return False
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            command=dict(type='str', choices=['config', 'flush', 'subordinate']),
            login_password=dict(type='str', no_log=True),
            login_host=dict(type='str', default='localhost'),
            login_port=dict(type='int', default=6379),
            main_host=dict(type='str'),
            main_port=dict(type='int'),
            subordinate_mode=dict(type='str', default='subordinate', choices=['main', 'subordinate']),
            db=dict(type='int'),
    ...
    # Subordinate Command section -----------
    if command == "subordinate":
        main_host = module.params['main_host']
        main_port = module.params['main_port']
        mode = module.params['subordinate_mode']

        # Check if we have all the data
        if mode == "subordinate":  # Only need data if we want to be subordinate
            if not main_host:
                module.fail_json(msg='In subordinate mode main host must be provided')

            if not main_port:
                module.fail_json(msg='In subordinate mode main port must be provided')
        ...
        info = r.info()
        if mode == "main" and info["role"] == "main":
            module.exit_json(changed=False, mode=mode)

        elif mode == "subordinate" and info["role"] == "subordinate" and info["main_host"] == main_host and info["main_port"] == main_port:
            status = dict(
                status=mode,
                main_host=main_host,
                main_port=main_port,
            ...
            if mode == "subordinate":
                if module.check_mode or\
                   set_subordinate_mode(r, main_host, main_port):
                    info = r.info()
                    status = {
                        'status': mode,
                        'main_host': main_host,
                    ...
                    module.fail_json(msg='Unable to set subordinate mode')

            else:
                if module.check_mode or set_main_mode(r):
                    module.exit_json(changed=True, mode=mode)
                else:
                    module.fail_json(msg='Unable to set main mode')
